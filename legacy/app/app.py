...
from webcharts.launcher import WebChartsLauncher
...
class AnalisisSACYLApp(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # Estado BD
        self.db: Optional[AnalysisDB] = None
    ...
    def _build_menu(self) -> None:
        ...
        m_view.add_command(
            label="Gráficas (nuevo)...",
            command=self.menu_open_web_charts,
        )
    ...
    # -----------------------------------------------------------------
    #   MENÚ VISTA: GRÁFICAS (NUEVO)
    # -----------------------------------------------------------------
    def menu_open_web_charts(self) -> None:
        if not self._db_is_open():
            messagebox.showwarning(
                "Gráficas (nuevo)",
                "No hay base de datos abierta.\n\n"
                "Abre o crea una base de datos desde 'Archivo'.",
            )
            return

        try:
            # Import diferido para no romper la app si faltan deps
            from webcharts.launcher import WebChartsLauncher
            launcher = WebChartsLauncher(str(self.db_path))
            base_url = launcher.start_api()
            launcher.open_dashboard(base_url)


        except ModuleNotFoundError as e:
            messagebox.showerror(
                "Gráficas (nuevo)",
                "Faltan dependencias para el dashboard web.\n\n"
                f"Detalle: {e}\n\n"
                "Solución: instala fastapi / uvicorn / pywebview "
                "en el intérprete de Python que usa PyCharm.",
            )
        except Exception as e:
            logger.exception("Error abriendo gráficas (nuevo)")
            messagebox.showerror(
                "Gráficas (nuevo)",
